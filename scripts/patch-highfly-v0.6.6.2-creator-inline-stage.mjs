...
// Final creator authority: this block is appended AFTER every historical HIGHFLY
// creator stylesheet. It does not change the 50/50 geometry owned by runtime; it
// only makes the class information use the available 425x149 area efficiently.
// Six attributes fit comfortably across 425 px, so using one row removes the
// exact extra line that caused the S23 class sheet to clip.
...
@media (orientation: landscape) and (max-height: 520px) {
  ...
  body.native-app #offline-select[data-hf-creator-step="class"] .hf-class-title,
...
console.log('[HIGHFLY v0.6.6.2] runtime-owned creator geometry + one-row class stats applied.');
